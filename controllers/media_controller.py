...
Handles upload, retrieval, serving, deletion, and association of media with properties and jobs.
"""
...
class MediaController:
    ...
    @login_required
    def associate_media_with_property(self, property_id, media_id):
        """
        Handle POST /properties/<property_id>/media/<media_id> endpoint.
        Associates media with a property.
        ...
        Returns:
            JSON response with success message or error.
        """
        # Authorization check - only admin can associate
        if current_user.role != 'admin':
            logger.warning(f"Unauthorized association attempt by user role: {current_user.role}")
            return jsonify({"error": "Unauthorized: Admin access required"}), 403
        ...
        # Create association
        association = self.media_service.associate_media_with_property(media_id, property_id)
        
        logger.info(f"Media {media_id} associated with property {property_id}")
        return jsonify({
            "message": "Media associated with property successfully",
    ...
    @login_required
    def disassociate_media_from_property(self, property_id, media_id):
        """
        Handle DELETE /properties/<property_id>/media/<media_id> endpoint.
        Disassociates media from a property.
        ...
        Returns:
            JSON response with success message or error.
        """
        # Authorization check - only admin can disassociate
        if current_user.role != 'admin':
            logger.warning(f"Unauthorized disassociation attempt by user role: {current_user.role}")
            return jsonify({"error": "Unauthorized: Admin access required"}), 403
        ...
        if success:
            logger.info(f"Media {media_id} disassociated from property {property_id}")
            return jsonify({
                "message": "Media disassociated from property successfully",
                "property_id": property_id,
                "media_id": media_id
            }), 200
        else:
    ...
    @login_required
    def associate_media_with_job(self, job_id, media_id):
        """
        Handle POST /jobs/<job_id>/media/<media_id> endpoint.
        Associates media with a job.
        ...
        # Create association
        association = self.media_service.associate_media_with_job(media_id, job_id)
        
        logger.info(f"Media {media_id} associated with job {job_id}")
        return jsonify({
            "message": "Media associated with job successfully",
            "job_id": job_id,
            "media_id": media_id,
            "association_id": association.id
        }), 200
    ...
    @login_required
    def disassociate_media_from_job(self, job_id, media_id):
        """
        Handle DELETE /jobs/<job_id>/media/<media_id> endpoint.
        Disassociates media from a job.
        ...
        if success:
            logger.info(f"Media {media_id} disassociated from job {job_id}")
            return jsonify({
                "message": "Media disassociated from job successfully",
                "job_id": job_id,
                "media_id": media_id
            }), 200
        else:
