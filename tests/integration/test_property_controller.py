...
class TestJobListFiltering:
    ...
    def apply_filters_and_test(self, property_id: int, start_date: date, end_date: date, show_completed: bool):
        ...
        assert len(job_cards) == len(expected_jobs), (
            f"Expected {len(expected_jobs)} incomplete jobs: {expected_jobs.keys()}, "
            f"got {len(job_cards)} {[job['data-job-id'] for job in job_cards]}"
        )
    ...
    def _date_range_combinations(self) -> list[tuple[date, date]]:
        ...
        return [
            ...
            if base_date + start_offset < base_date + end_offset
        ]
