...
def test_property_card_gallery(docker_admin_page) -> None:
    """
    ...
    """
    # Now run the UI test
    open_address_book(docker_admin_page)
    expect(docker_admin_page.locator("#property-list")).to_be_visible()
    expect(docker_admin_page.get_by_text("All Properties")).to_be_visible()

    property_card = get_first_property_card(docker_admin_page)
    expect(property_card).to_be_visible()

    open_property_card_gallery(docker_admin_page, property_card)
    
    # Assert gallery modal content - should show actual media now
    gallery_modal = docker_admin_page.locator("#media-gallery-modal")
