...
def upload_gallery_media(gallery_modal: Locator, media_paths: list[str]) -> None:
    ...
    # Some apps trigger confirm() immediately after files are chosen
    # Others do it after upload starts – so we wait for network activity
    page.wait_for_load_state("networkidle")
    ...
    #     uploading.first.wait_for(state="hidden", timeout=60_000)

    # ---------------------------
    # 5. Verify gallery contents
    # ---------------------------
    # gallery_modal.locator("#gallery-image, #gallery-video").first.wait_for(
